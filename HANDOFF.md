...
> **STALE IN ONE IMPORTANT WAY — corrected 2026-09-11. This file is
> GITIGNORED** (`.gitignore:1`, from `f4fa44a`), so it never appears in
> `git status`, nothing can ever contradict it, and it has drifted since
> 2026-08-20 with no mechanism to notice.
>
> **`~/.config/nvim` IS a git repository, and it HAS a public remote.**
> `github.com:pcmariani/nvim`, branch `main`, pushed. This file says the
> opposite in three places (the old text is kept below and marked, because the
> dated record is the point). Three commits landed on 2026-09-11 alone:
> `4ecccd8`, `19d6cc3`, `ce82d6e`. **Do not `git init` here and do not assume
> edits are uncommitted or unrecoverable.**
>
> Everything else below still reads true, but it describes 2026-08-20 and has
> not been re-verified since. **Check dates and process ages before trusting
> any of it** — `setup()` runs once at startup, so a claim about nvim's
> behaviour can be true of the config and false of every running instance.
...
3. ~~Still no git repo at `~/.config/nvim` — nothing from this or the prior session is committed. Raise with the user if they ever want version control here.~~ **FALSE as of 2026-09-11 — see the correction at the top. The repo exists, has a public remote, and is pushed.**

## Blockers / watch-outs

- ~~**`~/.config/nvim` is not a git repository.** None of this session's file edits are committed (there's nothing to commit to). If the user wants history/rollback for this config, that needs setting up from scratch.~~ **FALSE as of 2026-09-11 — see the correction at the top. It is a repo with a public remote and pushed history; nothing needs setting up.**
...
- No memory files were written this session (nothing here is a durable cross-project fact — all specific to this config's current state). ~~No commits exist (no git repo at this path).~~ **FALSE as of 2026-09-11 — see the correction at the top.**
